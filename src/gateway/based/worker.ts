...
const shardInfo = (shard: DiscordenoShard): WorkerShardInfo => ({
...
async function parentListener(data: WorkerMessage) {
	switch (data.type) {
		...
		case "SHARD_PAYLOAD":
			...
			break;
		case "GET_SHARD_INFO": {
			const info = [...manager.values()].map(shardInfo);
			const reply: MessageFromWorker = { type: "PONG_REPLY", pong: data.pong, data: info };

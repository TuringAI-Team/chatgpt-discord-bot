...
export default class MetricsCommand extends Command {
  ...
  public async run(interaction: CommandInteraction): CommandResponse {
    if (!this.bot.app.config.metrics)
      return new ErrorResponse({
        interaction,
        command: this,
        message: "Metrics are currently disabled",
      });

    /* Action to execute */
    const action: "save" | "view" = interaction.options.getSubcommandGroup(
      true
    ) as any;
    ...
    if (action === "save") {
      ...
      const type: "chart" | "raw" = interaction.options.getSubcommand(
        true
      ) as any;
      ...
      if (pending.length === 0)
        return new ErrorResponse({
          interaction,
          command: this,
          message: "There are no pending metrics",
        });

...
export default {
  ...
  async execute(interaction, client, commands, commandType, options) {
    let token = await useToken("chatgpt");
    ...
    if (!token) {
      ...
      let c: any = new OpenAIApi(configuration);
      let response = await c.createCompletion({
      ...
      let res = await format(response.data, false);
      await commandType.reply(interaction, {

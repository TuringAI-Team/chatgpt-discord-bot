...
export class BotManager extends EventEmitter {
  ...
  private formatWebhookEmbed(
    type: DiscordWebhookAnnounceType,
    cluster?: Cluster
  ): EmbedBuilder {
    return new EmbedBuilder()
      .setTitle(
        DiscordWebhookAnnounceTypeMap[type].replaceAll(
          "%",
          (cluster !== undefined ? cluster.id + 1 : -1).toString()
        )
      )
  ...
  private async announce(
    ...
    await this.rest
      .post(Routes.channelMessages(this.app.config.channels.status.channel), {
        body: {
          embeds: [embed.toJSON()],
        },
      })
  ...
  private async onDeath(cluster: Cluster): Promise<void> {
    this.app.logger.error(
      `Cluster ${chalk.bold(
        `#${cluster.id + 1}`
      )} experienced an error, attempting to restart.`
    );
  ...
  private async onReady(cluster: Cluster): Promise<void> {
    ...
    if (cluster.restarts.current > 0)
      await this.announce(DiscordWebhookAnnounceType.StartCluster, cluster);
  }
  ...
  private crashed(error: Error): void {
    this.app.logger.error(
      chalk.bold("The application crashed, with error ->"),
      error
    );

    this.announce(DiscordWebhookAnnounceType.CrashBot).then(() =>
      process.exit(1)
    );
  }
  ...
  public async startQueue(): Promise<void> {
    ...
    for (let i = 0; i < this.manager!.totalClusters; i++) {
      ...
      counter +=
        this.manager!.shardsPerClusters ?? this.app.config.shardsPerCluster;

      /* Spawn the next cluster. */
      if (this.app.config.dev)
        this.app.logger.debug(
          `Spawning cluster ${chalk.bold(`#${i + 1}`)} ...`
        );
  ...
  public async setup(): Promise<void> {
    ...
    console.log(`[REST] Initialized in ${Date.now() - now}ms.`);
    ...
    this.manager = new ClusterManager("build/bot/bot.js", {
      ...
      shardsPerClusters:
        typeof this.app.config.shardsPerCluster === "string"
          ? undefined
          : this.app.config.shardsPerCluster,
    ...
    console.log(`[ClusterManager] Initialized in ${Date.now() - now}ms.`);

    this.manager.extend(new ReClusterManager());
    ...
    if (this.app.config.dev)
      this.manager.on("debug", (line) => this.app.logger.debug(line));
    ...
    await this.fetchSession()
      ...
      .catch((error) => {
        this.app.logger.error(
          `Failed to fetch Discord session limit ->`,
          error
        );
        this.app.stop(1);
      });
    console.log(`[Session] Initialized in ${Date.now() - now}ms.`);
    /* Launch the actual sharding manager. */
    await this.startQueue().catch((error) => {
      this.app.logger.error(`Failed to start clusters in queue ->`, error);
      this.app.stop(1);
    });
    console.log(`[Queue] Initialized in ${Date.now() - now}ms.`);
    ...
    this.app.logger.debug(
      ...
      )} for ${`${chalk.bold(this.clusters.size)} cluster${
        this.clusters.size > 1 ? "s" : ""
      }`} to be initialized.`
    );
    this.app.logger.info("Up n' running!");

    if (!this.app.config.dev)
      await this.announce(DiscordWebhookAnnounceType.StartBot);
    this.started = true;
  }
}
